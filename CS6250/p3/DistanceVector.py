...
class DistanceVector(Node):
    ...
    def __init__(self, name, topolink, outgoing_links, incoming_links):
        ...
        #I decided to use a dictonary because you want to keep track of the distance relative to the node
        self.cost = {name: 0}
    ...
    def send_initial_messages(self):
        ...
        for i in self.neighbor_names:
            self.send_msg((self.name,self.cost.items()),i)
    ...
    def process_BF(self):
        ...
        for msg in self.messages:
            nodeName = msg[0]
            nodeDistance = msg[1]
            costOfNode = self.get_outgoing_neighbor_weight(msg[0])
            for n, c in nodeDistance:
                if c != -99:
                    c += int(costOfNode)
                if c < -99:
                    c = -99
                if n == self.name:
                    continue
                if n not in self.cost or c < self.cost[n]:
                    self.cost[n] = c
                    upDate = True
                if c == -99:
                    c == -99
